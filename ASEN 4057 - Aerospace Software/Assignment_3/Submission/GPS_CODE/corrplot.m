...
% reflect_Q= 0
% Reflect_Composite= 0
% Reflect_Direct_Ratio= 0

function [] = corrplot(avgnum,direct_I,direct_Q, reflect_I,reflect_Q, Reflect_Direct_Ratio, Reflect_Composite)
...
%     subplot(241),
    %I Prompt
    plot(direct_I,spacing2,di2,'r.');
    hold(direct_I,'on');
    plot(direct_I,spacing2,di2,'c');
    hold(direct_I,'off');
    axis([(spacing2(1)-0.1) (spacing2(end)+0.1) (dmin-0.05) 1.05])
    title(direct_I,'direct (I)')
    %Q Prompt
    plot(direct_Q,spacing2,dq2,'g.');
    hold(direct_Q,'on');
    plot(direct_Q,spacing2,dq2,'c');
    hold(direct_Q,'off');
    axis([(spacing2(1)-0.1) (spacing2(end)+0.1) (dmin-0.05) 1.05])
    title(direct_Q,'direct (Q)')
    
    %Reflected direct ratio plotting
    plot(Reflect_Direct_Ratio,spacing2,d3,'b.');
    hold(Reflect_Direct_Ratio,'on');
    plot(Reflect_Direct_Ratio,spacing2,d3,'m');
    hold(Reflect_Direct_Ratio,'off')
    axis([(spacing2(1)-0.1) (spacing2(end)+0.1) (d3min-0.05) 1.05])
    powerrat(loopcnt) = rrmaxrat / ddmax;
    
    %Reflected Signal Plotting
    title(Reflect_Direct_Ratio,['reflected/direct ratio: ',num2str(powerrat(loopcnt))])
    rmin=min([ri2 rq2]);
    r3min=min([r3]);
    plot(reflect_I,-spacing(end:-1:1),ri2,'r.');
    hold(reflect_I,'on');
    plot(reflect_I,-spacing(end:-1:1),ri2,'c');
    hold(reflect_I,'off');
    axis([(-spacing(end)-0.1) (-spacing(1)+0.1) (rmin-0.05) 1.05])
    title(reflect_I,'reflect (I)')
    plot(reflect_Q,-spacing(end:-1:1),rq2,'g.');
    hold(reflect_Q,'on')
    plot(reflect_Q,-spacing(end:-1:1),rq2,'c');
    hold(reflect_Q, 'off')
    axis([(-spacing(end)-0.1) (-spacing(1)+0.1)  (rmin-0.05) 1.05])
    title(reflect_Q,'reflect (Q)')
    
    %Composite Reflected Signal Plotting
    plot(Reflect_Composite,-spacing(end:-1:1),r3,'b.');
    hold(Reflect_Composite, 'on')
    plot(Reflect_Composite,-spacing(end:-1:1),r3,'m');
    hold(Reflect_Composite,'off')
    ...
    title(Reflect_Composite,['reflected composite; path delay of ',num2str(pathdelay(loopcnt),6),'m '])
    xlabel(Reflect_Composite,['time elapsed : ',num2str(loopcnt*avgnum/(60*1000),4),' min; Int#:',int2str(loopcnt)])
...
figure(200)
% subplot(2,1,1),
plot((1:loopcnt)*(avgnum/1000),pathdelay,'c');hold on
% subplot(2,1,1),
plot((1:loopcnt)*(avgnum/1000),pathdelay,'c.');
title(['Estimated Path Delay for Reflected Signal (',int2str(avgnum),' msec avg)'])
ylabel('meters')
xlabel('seconds')
grid
% subplot(2,1,2),
plot((1:loopcnt)*(avgnum/1000),powerrat,'b.');hold on
